...
async function loadCustomerRewards(){const p=await guardRole(["customer"]);if(!p)return;const box=$("rewards");const {data,error}=await sb.rpc('get_customer_coupons',{p_token:p.loyalty_token});if(error){box.innerHTML=`<div class="alert">${esc(error.message)}</div>`;return;}const coupons=Array.isArray(data)?data:[];const available=coupons.filter(c=>String(c.status||'').toUpperCase()==='AVAILABLE');const obtained=coupons.filter(c=>['REDEEMED','USED','OBTAINED'].includes(String(c.status||'').toUpperCase()));const total=coupons.length;const a=$("rewardAvailable"),o=$("rewardObtained"),t=$("rewardTotal");if(a)a.textContent=available.length;if(o)o.textContent=obtained.length;if(t)t.textContent=total;const list=$("rewardHistory");if(!list)return;if(!coupons.length){list.innerHTML='<div class="empty">No rewards yet. Earn 10 visits to unlock your first reward.</div>';return;}const sorted=[...coupons].sort((x,y)=>new Date(y.created_at||0)-new Date(x.created_at||0));list.innerHTML=sorted.map(c=>{const status=String(c.status||'AVAILABLE').toUpperCase();const label=status==='AVAILABLE'?'AVAILABLE':status==='REDEEMED'||status==='USED'?'OBTAINED':status;const code=esc(c.coupon_code||'Reward');const date=c.created_at?new Date(c.created_at).toLocaleDateString():'';return `<div class="reward"><div class="food">☕</div><div class="grow"><strong>Free Drink</strong><small>${code}${date?` • ${date}`:''}</small></div><span class="badge">${label}</span></div>`;}).join('');}
...
async function loadAddPoints(){await loadStaffCustomer();const btn=$("addPointsBtn");if(!btn)return;btn.onclick=async()=>{const c=getStaffCustomer();if(!c){toast('Customer not found.',false);return;}btn.disabled=true;btn.textContent='Adding 1 visit…';try{const customerCode=c.customer_code||c.member_id;if(!customerCode)throw new Error('Customer code is missing.');const {data,error}=await sb.rpc('add_customer_point',{p_customer_code:customerCode});if(error)throw error;const updated=Array.isArray(data)?data[0]:data;if(updated&&typeof updated==='object'){Object.assign(c,updated);}else if(typeof updated==='number'){c.points=updated;}c.points=Number(c.points||0);saveStaffCustomer(c);const earned=c.points>0&&c.points%10===0;toast(earned?'1 visit added — reward unlocked!':'1 visit added successfully.');setTimeout(()=>location.href='staff-customer.html',650);}catch(e){toast(e.message||'Could not add visit.',false);}finally{btn.disabled=false;btn.textContent='ADD 1 VISIT';}};}
async function loadStaffRedeem(){const p=await guardRole(['staff','gm']);if(!p)return;const c=getStaffCustomer();if(!c){location.href='staff-scan.html';return;}renderStaffCustomer(c);const box=$("rewards");const token=c.loyalty_token;if(!token){box.innerHTML='<div class="alert">Customer QR token is missing.</div>';return;}const {data,error}=await sb.rpc('get_customer_coupons',{p_token:token});if(error){box.innerHTML=`<div class="alert">${esc(error.message)}</div>`;return;}const available=(Array.isArray(data)?data:[]).filter(x=>String(x.status||'').toUpperCase()==='AVAILABLE');box.innerHTML=available.length?available.map(coupon=>`<div class="reward"><div class="food">☕</div><div class="grow"><strong>Free Drink</strong><small>${esc(coupon.coupon_code||'Reward')} • Ready to use</small></div><button class="btn primary" onclick="redeemReward('${esc(coupon.coupon_code||'')}')">Redeem</button></div>`).join(''):'<div class="empty">No rewards available for this customer.</div>';}
async function redeemReward(couponCode){const c=getStaffCustomer();if(!c||!couponCode)return;try{const customerCode=c.customer_code||c.member_id;if(!customerCode)throw new Error('Customer code is missing.');const {data,error}=await sb.rpc('redeem_customer_coupon',{p_customer_code:customerCode,p_coupon_code:couponCode});if(error)throw error;const r=Array.isArray(data)?data[0]:data;if(r&&typeof r==='object')Object.assign(c,r);saveStaffCustomer(c);toast(`${couponCode} redeemed successfully.`);setTimeout(()=>location.href='staff-customer.html',700);}catch(e){toast(e.message||'Could not redeem reward.',false);}}
