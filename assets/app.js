...
async function loadCustomerRewards(){const p=await guardRole(["customer"]);if(!p)return;const box=$("rewards");const {data,error}=await sb.from('rewards').select('id,name,description,points_cost,is_active').eq('is_active',true).order('points_cost');if(error){box.innerHTML=`<div class="alert">${esc(error.message)}</div>`;return;}box.innerHTML=data?.length?data.map(r=>`<div class="reward"><div class="food">★</div><div class="grow"><strong>${esc(r.name)}</strong><small>${esc(r.description||'Al Noor reward')} • ${moneyPoints(r.points_cost)} points</small></div><span class="badge">${moneyPoints(r.points_cost)} pts</span></div>`).join(''):`<div class="empty">No rewards are active yet.</div>`;}
...
function renderStaffCustomer(c){if(!c)return;document.querySelectorAll('[data-name]').forEach(e=>e.textContent=c.full_name||'Customer');document.querySelectorAll('[data-id]').forEach(e=>e.textContent=c.member_id||'—');document.querySelectorAll('[data-points]').forEach(e=>e.textContent=moneyPoints(c.points));const a=$("avatar");if(a)a.textContent=initials(c.full_name);const s=$("customerStatus");if(s)s.textContent=c.is_active?'Active':'Inactive';}
...
async function loadAddPoints(){await loadStaffCustomer();const btn=$("addPointsBtn");if(!btn)return;btn.onclick=async()=>{const c=getStaffCustomer(),points=Number($("pts")?.value||0),note=$("note")?.value.trim()||null;if(!c||points<=0){toast('Enter valid points.',false);return;}btn.disabled=true;btn.textContent='Adding…';try{const staff=await getProfile();const {data,error}=await sb.rpc('add_customer_point',{p_customer_id:c.id,p_points:points,p_location_id:staff?.location_id||null,p_note:note});if(error)throw error;c.points=typeof data==='number'?data:(Array.isArray(data)?data[0]:data);saveStaffCustomer(c);toast(`${points} points added successfully.`);setTimeout(()=>location.href='staff-customer.html',500);}catch(e){toast(e.message||'Could not add points.',false);}finally{btn.disabled=false;btn.textContent='Add Points →';}};}
async function loadStaffRedeem(){const p=await guardRole(['staff','gm']);if(!p)return;const c=getStaffCustomer();if(!c){location.href='staff-scan.html';return;}renderStaffCustomer(c);const box=$("rewards"),{data,error}=await sb.from('rewards').select('id,name,description,points_cost,is_active').eq('is_active',true).order('points_cost');if(error){box.innerHTML=`<div class="alert">${esc(error.message)}</div>`;return;}box.innerHTML=data?.length?data.map(r=>`<div class="reward"><div class="food">★</div><div class="grow"><strong>${esc(r.name)}</strong><small>${esc(r.description||'Reward')} • ${moneyPoints(r.points_cost)} points</small></div><button class="btn primary" onclick="redeemReward('${r.id}',${Number(r.points_cost)})">Redeem</button></div>`).join(''):`<div class="empty">No rewards are active.</div>`;}
async function redeemReward(id,cost){const c=getStaffCustomer();if(!c)return;if(Number(c.points)<Number(cost)){toast('Customer does not have enough points.',false);return;}try{const staff=await getProfile();const {data,error}=await sb.rpc('redeem_customer_coupon',{p_customer_id:c.id,p_reward_id:id,p_location_id:staff?.location_id||null});if(error)throw error;const r=Array.isArray(data)?data[0]:data;c.points=r?.remaining_points??(c.points-cost);saveStaffCustomer(c);toast(`Reward redeemed${r?.coupon_code?`. Coupon: ${r.coupon_code}`:''}`);setTimeout(()=>location.href='staff-customer.html',700);}catch(e){toast(e.message||'Could not redeem reward.',false);}}
